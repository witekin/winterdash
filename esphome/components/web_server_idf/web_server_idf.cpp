...
void AsyncEventSource::handleRequest(AsyncWebServerRequest *request) {
  ...
  // WinterDash "latest-viewer-wins" SSE cap for tight-heap boards (no-PSRAM CYD): a 2nd concurrent
  // SSE client fragments the 8-bit heap IRREVERSIBLY (measured: blk8 11KB->1.1KB on the 2nd tab,
  // pinned until reboot) -> LVGL glyph OOM / HA drop / web wedge. Evict the OLDEST live session(s)
  // BEFORE admitting the new one so we never hold more than WD_SSE_MAX_VIEWERS full clients.
  // Only the async httpd_sess_trigger_close is called here: fd_ is left for the destroy callback to
  // clear and WebServer::loop() to reap (do NOT delete or zero fd_ here — that races httpd's socket
  // teardown / free_ctx). The eviction is a single oldest-first pass (sessions_ is push_back-ordered),
  // so no re-selection despite fd_ staying set until the async close lands. Board-gated: boards that
  // don't define WD_SSE_MAX_VIEWERS keep unlimited multi-viewer (headless has the heap for it).
  {
    int live = 0;
    for (auto *ses : this->sessions_)
      if (ses->fd_.load() != 0)
        live++;
    int to_evict = live - (WD_SSE_MAX_VIEWERS - 1);  // make room for the one we're about to admit
    for (auto *ses : this->sessions_) {
      if (to_evict <= 0)
        break;
      if (ses->fd_.load() != 0) {
        ESP_LOGI(TAG, "WD SSE cap: evicting oldest viewer (fd %d, live %d, max %d)", ses->fd_.load(), live,
                 WD_SSE_MAX_VIEWERS);
        httpd_sess_trigger_close(ses->hd_, ses->fd_.load());
        to_evict--;
      }
    }
  }
